...
class OrderHandler:
    ...
    def __init__(self, main_currency, second_currency, binance_api, strategy_id):
        ...
        if current_trade > self.last_trade_timeout:
            if not self.own_main:
                ...
                if production:
                    ...
                    self.database_connector.insert_order(self.strategy_id, "BUY", self.main_currency,
                                                         self.second_currency,
                                                         self.binance_api.get_latest_price(
                                                             self.main_currency + self.second_currency),
                                                         datetime.datetime.now(), production)
